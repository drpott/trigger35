...
def device_match(name):
    ...
    try:
        ...
        if matches:
            ...
            choice = eval(input('Enter a device number: ')) - 1
...
class NetDevices(DictMixin):
    ...
    class _actual(object):
        ...
        def find(self, key):
            ...
            matches = [x for x in list(self.keys()) if x.startswith(key + '.')]
        ...
        def get_devices_by_type(self, devtype):
            ...
            """
            return [x for x in list(self.values()) if x.deviceType == devtype]
    ...
    def __iter__(self):
        # nd._Singleton.__dict__['_actual__dict']['fortinet']

        return iter(self.__class__._Singleton.__dict__)

    def __delitem__(self, key):
        del self.__class__._Singleton.__dict__[key]
        
    def __setitem__(self, key, value):
        self.__class__._Singleton.__dict__[key] = value
        
    def __getitem__(self, key):
        return self.__class__._Singleton.__dict__[key]

    def __len__(self):
        return len(self.__class__._Singleton.__dict__['_actual__dict'])
